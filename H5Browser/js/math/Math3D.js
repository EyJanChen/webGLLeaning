class Math3D {
   ...
   * multiplying the matrices together.  For given a and b, this static returns
   ...
   */
  static distance(a, b) {
    return Math.sqrt(Math3D.distanceSq(a, b));
  }
  ...
  static lookAt(cameraPosition, target, up, dst) {
    dst = dst || new Float32Array(16);
    let zAxis = Math3D.normalize(Math3D.subtractVectors(cameraPosition, target, null), null);
    let xAxis = Math3D.normalize(Math3D.cross(up, zAxis, null), null);
    let yAxis = Math3D.normalize(Math3D.cross(zAxis, xAxis, null), null);
